...
// ── Sidebar nav items — chaque entrée a un href pour naviguer ──────────────────
const navItems = [
  { id: "accueil",    label: "Tableau de bord",  icon: HomeIcon,      href: "/" },
  { id: "elections",  label: "Élections 2026 🗳️", icon: Vote,          href: "/elections" },  // ← nouveau
  { id: "actualites", label: "Actualités",        icon: FileText,      href: "/actualites" },
  { id: "bilan",      label: "Bilan de Mandat",   icon: BarChart3,     href: "/bilan-mandat" },
  { id: "droits",     label: "Vos Droits",        icon: Shield,        href: "/vos-droits" },
  { id: "documents",  label: "Documents Utiles",  icon: FileText,      href: "/documents-utiles" },
  { id: "faq",        label: "FAQ",               icon: HelpCircle,    href: "/faq" },
  { id: "contact",    label: "Nous Contacter",    icon: MessageSquare, href: "/contact" },
];
        ...
        <div className="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16 lg:h-18">
            ...
            <div className="flex items-center gap-2">
              ...
              <button onClick={() => navigate("/admin/login")} className="p-2.5 hover:bg-slate-100 rounded-lg transition-colors" title="Connexion admin">
                <User className="w-5 h-5 text-slate-500" />
              </button>
              {/* ✅ Bouton "Voter" dans le header pendant la période électorale */}
              <Link
                to="/elections"
                className="hidden sm:flex items-center gap-1.5 bg-red-600 hover:bg-red-700 text-white rounded-full px-4 py-2 text-sm font-bold shadow-md shadow-red-200 transition-colors"
              >
                <span className="relative flex h-2 w-2">
                  <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-white opacity-75" />
                  <span className="relative inline-flex rounded-full h-2 w-2 bg-white" />
                </span>
                Voter jusqu'au 6 mai
              </Link>
              <Button className="bg-red-600 hover:bg-red-700 text-white rounded-full px-5 py-2.5 text-sm font-semibold shadow-md shadow-red-200 hidden sm:flex" onClick={() => navigate("/adhesion")}>
                <Users className="w-4 h-4 mr-2" />Nous rejoindre
              </Button>
            </div>
          </div>
        </div>
      ...
      <div className="flex max-w-[1440px] mx-auto">
        ...
        `}>
          <div className="p-4 space-y-1">
            {navItems.map((item) => (
              // ✅ Chaque item navigue maintenant vers sa page
              <Link
                key={item.id}
                to={item.href}
                onClick={() => { setActiveSection(item.id); setSidebarOpen(false); }}
                className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all duration-200 ${
                  // Surligné si Elections et on est sur /elections (ou accueil par défaut)
                  item.id === "elections"
          ...
          <div className="mx-4 mt-6 p-4 bg-slate-50 rounded-xl border border-slate-100">
            ...
            <p className="text-xs text-slate-600 italic leading-relaxed">Seul on va plus vite, ensemble on va plus loin.</p>
            <p className="text-xs font-bold text-red-600 mt-2">FOCOM UES ILIAD</p>
          </div>

          <div className="p-4 mt-4 flex gap-2">
            <a href="https://www.facebook.com/groups/focomiliad/" target="_blank" rel="noopener noreferrer" className="p-2 bg-slate-100 rounded-lg hover:bg-slate-200 transition-colors">
        ...
        <main className="flex-1 min-w-0 p-4 lg:p-8">
          ...
          {/* ✅ Bannière élections — mise en avant visible dès l'accueil */}
                ...
                },
                {
                  ...
                  revealContent: (
                    <div className="space-y-3">
                      ...
                      {/* ✅ Ce bouton navigue maintenant vers /elections */}
                ...
                <div className="space-y-4">
                  {[
                    ...
                    },
                    {
                      ...
                      date: "29 octobre 2026",
                      category: "Élections",
                      href: "/elections", // ✅ lien vers la page élections
